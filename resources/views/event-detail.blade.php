        ...
        <div class="lg:col-span-1">
            <div class="sticky top-32">
                <img src="{{ ($event->poster_path && \Illuminate\Support\Facades\Storage::disk('public')->exists($event->poster_path))
                        ...
                        : 'https://placehold.co/200x600' }}" alt="{{ $event->title }} Poster"
                ...
                <div class="mt-8 p-6 bg-white rounded-3xl border border-slate-100 shadow-sm">
                    ...
                    <div class="flex items-center gap-4">
                        <div class="w-12 h-12 bg-indigo-100 rounded-full flex items-center justify-center text-indigo-600 font-bold">
                            {{ strtoupper(substr($event->category->name ?? 'EV', 0, 2)) }}</div>
                        <div>
                            <p class="font-bold text-slate-800">{{ $event->category->name ?? 'Tanpa Kategori' }}</p>
                            <p class="text-xs text-slate-500">Detail acara dari database</p>
                        </div>
                    </div>
                </div>
            </div>
